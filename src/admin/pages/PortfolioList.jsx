...
import { usePortfolio, deletePortfolioProject } from '../hooks/usePortfolio'
...
const PortfolioList = () => {
  ...
  const { rows, count, loading, reload } = usePortfolio({ search, status, categoryId, from, to })
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePortfolioProject(deleting.id)
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-slate/70">{(row.tech_stack ?? []).join(', ')}</p>
          </div>
        </div>
      ),
    },
    ...
    { key: 'updated_at', label: 'Updated', render: (row) => format(new Date(row.updated_at), 'MMM d, yyyy') },
    {
      ...
      render: (row) => (
        <div className="flex justify-end gap-2">
          <button
            type="button"
            onClick={() => navigate(`/admin/portfolio/${row.id}/edit`)}
            aria-label="Edit"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-navy">Portfolio Projects</h1>
          <p className="mt-1 text-sm text-slate">{count} project{count === 1 ? '' : 's'}</p>
        </div>
        <Button as={Link} to="/admin/portfolio/new" icon={Plus}>
          New Project
        </Button>
      </div>
      ...
      <DataTable
        ...
        emptyDescription="Add your first project to showcase your work."
        emptyAction={
          <Button as={Link} to="/admin/portfolio/new" icon={Plus}>
            New Project
          </Button>
